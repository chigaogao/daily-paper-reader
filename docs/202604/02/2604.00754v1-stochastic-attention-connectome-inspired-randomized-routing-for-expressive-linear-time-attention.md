...
tldr: "受果蝇全脑连接组中随机捷径实现高效全局通信的启发，本文提出随机注意力（Stochastic Attention, SA）。该方法在滑动窗口注意力（SWA）基础上，通过在计算前对序列进行随机置换并在计算后恢复，将局部窗口转化为随机全局窗口。SA在保持线性复杂度的同时，使感受野随深度呈指数级增长。实验证明，SA在语言模型预训练和推理任务中均优于传统SWA，为提升高效注意力的表达能力提供了新思路。"
selection_source: fresh_fetch
motivation: 借鉴生物脑连接组中稀疏但高效的随机长程连接，旨在解决滑动窗口注意力感受野增长缓慢且缺乏全局通信的问题。
method: 在滑动窗口注意力前后引入随机置换与逆置换，以线性复杂度将固定局部窗口转化为随机全局路由。
result: 实验显示 SA 在预训练零样本准确率和 Qwen3 模型推理性能上均显著优于 SWA，并能匹配更复杂的注意力机制。
conclusion: 受连接组启发的随机路由是提升高效注意力表达能力的实用原语，为构建线性时间长序列模型提供了新方向。
---
