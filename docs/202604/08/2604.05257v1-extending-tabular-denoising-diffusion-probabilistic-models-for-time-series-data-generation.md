...
tldr: 本研究针对 TabDDPM 无法处理时间依赖性的局限，提出了一种用于时间序列生成的扩展模型。通过引入轻量级时间适配器和上下文感知嵌入模块，将传感器数据重构为窗口序列，并显式建模时间上下文。该方法在 WISDM 加速度计数据集上验证了其生成的合成序列在时间真实性、多样性和相干性方面表现优异，且分类性能与真实数据相当，为隐私保护的数据增强提供了有效方案。
selection_source: fresh_fetch
motivation: 传统的 TabDDPM 假设样本之间相互独立，难以捕捉时间序列数据中至关重要的时间依赖关系。
method: 通过引入轻量级时间适配器、上下文感知嵌入模块及窗口化序列重构，增强了模型对时间上下文的建模能力。
result: 在 WISDM 数据集上，该模型生成的合成数据在分类性能上与真实数据相当，且在时间真实性和统计对齐方面优于基准方法。
conclusion: 扩散模型在引入时间推理机制后，能够成为生成高质量、具有序列一致性的合成数据的有效且灵活的工具。
---
