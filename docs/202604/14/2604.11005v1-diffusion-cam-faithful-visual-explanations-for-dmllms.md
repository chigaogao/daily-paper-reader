...
title_zh: Diffusion-CAM：针对扩散多模态大语言模型的高保真视觉解释
...
tldr: 针对扩散多模态大语言模型（dMLLMs）并行去噪生成的特性，传统自回归解释方法（如CAM）难以捕捉其平滑且分布式的激活模式。本文提出Diffusion-CAM，这是首个专为dMLLMs设计的可解释性方法。该方法通过微分探测Transformer骨干网络的中间表示，结合梯度信息并利用四个关键模块解决空间歧义和冗余相关性，显著提升了视觉解释的准确性与保真度。
selection_source: fresh_fetch
motivation: 传统CAM方法适用于序列依赖的自回归模型，难以解释dMLLM中非自回归、并行的去噪生成行为。
method: 通过微分探测Transformer中间层的潜在特征与类别梯度，并引入四个模块来消除随机性、空间歧义及冗余相关性。
result: 实验结果显示，Diffusion-CAM在定位精度和视觉保真度方面均显著超越了现有的SOTA解释方法。
conclusion: Diffusion-CAM为理解扩散多模态系统的并行生成机制提供了有效工具，并建立了该领域解释性的新标准。
---

## 摘要
虽然扩散多模态大语言模型（dMLLMs）近期在多模态生成领域取得了显著进展，但其可解释性机制的发展却滞后于其架构演进。与产生序列激活的传统自回归模型不同，基于扩散的架构通过并行去噪生成 token，从而在整个序列中产生平滑且分布式的激活模式。因此，现有的类激活映射（CAM）方法是为局部序列依赖性量身定制的，并不适用于解释这些非自回归行为。为了填补这一空白，我们提出了 Diffusion-CAM，这是首个专门为 dMLLMs 定制的可解释性方法。我们通过对 Transformer 骨干网络中的中间表示进行可微探测来获取原始激活图，从而捕获潜在特征及其特定类别的梯度。为了解决这些原始信号固有的随机性，我们引入了四个关键模块，以解决空间歧义并减轻图像内混杂因素和冗余 token 相关性的影响。广泛的实验表明，Diffusion-CAM 在定位准确性和视觉保真度方面均显著优于现有最先进（SoTA）方法，为理解扩散多模态系统的并行生成过程建立了新标准。
