...
title_zh: 无需 ODE 的神经流匹配用于单步生成建模
authors: Xiao Shou
date: 2026-04-07
...
## 摘要
扩散模型和流匹配模型通过学习随时间变化的向量场来生成样本，这些向量场的积分将噪声传输到数据，在推理时需要数十次到数百次网络评估。相比之下，我们直接学习传输映射。我们提出了最优传输神经流匹配（OT-NFM），这是一种无需 ODE 的生成框架，它使用神经流对流映射进行参数化，从而通过单次前向传递实现真正的单步生成。我们展示了朴素的流映射训练会遭受均值崩溃（mean collapse），即不一致的噪声-数据配对会导致所有输出都趋向于数据均值。我们证明了一致耦合对于非退化学习是必要的，并利用具有可扩展小批量和在线耦合策略的最优传输配对来解决这一问题。在合成基准测试和图像生成任务（MNIST 和 CIFAR-10）上的实验表明，该方法在保持具有竞争力的样本质量的同时，将推理过程简化为单次网络评估。
