...
tldr: 本文提出了一种广义扩散模型框架，将传统的独立噪声注入替换为包含相互作用结构的马尔可夫链蒙特卡罗（MCMC）动力学。通过引入Ising耦合，该方法能有效利用目标系统的空间相关性。该框架可无缝映射至基于概率比特（p-bits）的概率计算机，在采样效率和能效上显著优于传统GPU，为处理复杂物理系统提供了高效的生成建模新途径。
selection_source: fresh_fetch
motivation: 传统扩散模型依赖独立噪声采样，难以高效捕捉具有强空间相关性系统的内在结构。
method: 通过在扩散动力学中显式引入Ising耦合，将独立扩散推广为包含结构化MCMC采样的相关扩散，并利用概率硬件加速计算。
result: 在2D Ising模型和3D自旋玻璃模型上的实验证明，相关扩散生成的样本在准确性上优于传统的独立扩散方法。
conclusion: 该框架展示了概率计算机在实现新型扩散算法方面的优势，证明了结构化采样能显著提升生成模型的性能和效率。
---
