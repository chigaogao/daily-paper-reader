...
## 摘要
现代传感系统产生海量的无标签多元时间序列数据。这种丰富的无标签数据使得自监督学习（SSL）成为学习可迁移表示的一种自然方法。然而，大多数现有方法针对重建或预测目标进行优化，往往无法捕捉下游分类和推理任务所需的语义结构。虽然最近的传感器-语言对齐方法通过字幕生成和零样本迁移提高了语义泛化能力，但它们受限于固定的传感器配置，如预定义的通道集、信号长度或时间分辨率，这阻碍了跨领域的适用性。为了填补这些空白，我们引入了 SLIP（传感器语言引导的预训练），这是一个开源框架，用于学习可在多种传感器设置中泛化的语言对齐表示。SLIP 将对比对齐与传感器条件下的字幕生成相结合，促进了判别性理解和生成式推理。通过交叉注意力机制重新利用预训练的仅解码器（decoder-only）语言模型，并引入一种简洁且灵活的补丁嵌入器（patch-embedder），SLIP 在推理时支持不同的时间分辨率和变长输入，而无需额外的重新训练。在 11 个数据集上，SLIP 在零样本迁移、信号字幕生成和问答任务中表现出卓越的性能。它实现了 77.14% 的平均线性探测准确率，比强基准模型相对提升了 5.93%，并在基于传感器的问答中达到了 64.83% 的准确率。
