...
tldr: 针对医疗等隐私敏感领域的多模态时间序列分类，本文提出一种数据本地化的LLM引导神经架构搜索（NAS）框架。该框架在严格的数据本地约束下运行，LLM控制器仅通过摘要信息（如指标和日志）进行决策，无需接触原始数据。通过联合搜索专家架构与预处理策略，该方法在UEA30和SleepEDFx数据集上显著提升了模型性能，减少了人工干预。
selection_source: fresh_fetch
motivation: 敏感时间序列数据受限于隐私保护，难以在云端进行高效的架构搜索和模型迭代。
method: 提出一种数据本地化的LLM引导NAS框架，采用“一对多”二进制专家模型和轻量级融合MLP，仅通过试验摘要信息进行远程控制。
result: 在UEA30和SleepEDFx数据集上的实验表明，该方法优于强基准模型，且性能达到了现有文献的发布水平。
conclusion: 该框架在确保数据隐私的前提下实现了自动化的架构搜索，有效降低了多模态时间序列建模中的人工干预成本。
---
