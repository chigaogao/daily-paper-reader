...
score: 7.0
evidence: 脑机接口分类架构
tldr: 本研究提出变分相量电路（VPC），这是一种受变分量子电路启发、在S1单位圆流形上运行的确定性经典学习架构。VPC通过可训练相移、局部酉混合和复空间干涉取代传统实值权重矩阵，为脑机接口信号分类提供了一种相位原生的统一方法。实验表明，VPC在保持竞争力的分类准确率的同时，显著减少了参数量，为相位干涉驱动的神经计算提供了数学原理支撑，并可作为未来量子混合系统的编码层。
...
conclusion: VPC证明了单位圆相位干涉是密集神经计算的有效替代方案，具有作为独立分类器或量子前端编码层的潜力。
---
