...
tldr: 本研究针对数字人建模（DHM）中平台依赖性强、可重复性差的问题，提出了一种跨平台的、支持AI的多模态数字人建模框架。该框架通过OpenBCI Galea头戴设备集成EEG、EMG等多种生理信号，并结合SuperTux游戏环境实现交互建模。其核心贡献在于实现了感知、交互建模与推理准备的解耦，为无障碍设计和自适应系统研究提供了可扩展、高同步性的基础设施。
selection_source: fresh_fetch
motivation: 现有的数字人建模方法往往与特定平台或任务紧密耦合，限制了研究的复用性、扩展性及伦理重用。
method: 提出一种解耦感知、交互与推理的框架，利用OpenBCI Galea采集多模态生理数据，并结合基于游戏的交互环境进行时间对齐。
result: 技术验证确认了该框架在数据完整性、流连续性和多传感器同步方面的可靠性，支持高吞吐量和低延迟的数据处理。
conclusion: 该框架为未来符合伦理要求的、包容性的数字人建模及人机交互研究提供了一个技术领先且灵活的基础设施。
---

## 摘要
数字人建模 (DHM) 正日益受到人工智能 (AI)、可穿戴生物传感和交互式数字环境进步的影响，特别是在解决可访问性和包容性的研究中。然而，许多赋能 AI 的 DHM 方法仍然与特定平台、任务或解释管线紧密耦合，限制了可重复性、可扩展性和伦理复用。本文提出了一种平台无关的 DHM 框架，旨在通过明确分离传感、交互建模和推理就绪性，来支持面向 AI 的多模态交互研究。该框架集成了 OpenBCI Galea 头显作为统一的多模态传感层，提供并发的脑电 (EEG)、肌电 (EMG)、眼电 (EOG)、光电容积脉搏波 (PPG) 和惯性数据流，以及使用 SuperTux 实现的可重复的基于游戏的交互环境。生理信号被表示为结构化的、时间对齐的可观测值，而不是直接嵌入 AI 模型或行为推理，从而使下游 AI 方法能够在获得适当伦理批准的情况下得以应用。交互使用计算任务原语和带时间戳的事件标记进行建模，支持跨异构传感器和平台的一致对齐。通过作者自我测试进行的技术验证确认了数据完整性、流连续性和同步性；文中未报告人类受试者评估或 AI 推理。本文还讨论了关于数据吞吐量、延迟以及扩展到其他传感器或交互模态的可扩展性考量。示例用例展示了该框架如何在无需修改架构的情况下，支持赋能 AI 的 DHM 和人机交互 (HCI) 研究，包括面向可访问性的交互设计和自适应系统研究。所提出的框架为未来经过伦理批准的、包容性的 DHM 研究提供了一个专注于新兴技术的基础设施。
