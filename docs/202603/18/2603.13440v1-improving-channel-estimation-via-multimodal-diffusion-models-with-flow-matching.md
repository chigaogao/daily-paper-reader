...
motivation: 传统信道估计在稀疏导频下表现不佳，且现有生成模型未能充分利用现代网络中丰富的多模态环境信息。
method: 提出一种基于流匹配和DiT的框架，融合多模态环境语义与稀疏导频结构信息来重建高保真信道。
result: 实验表明该方法在多种导频密度下均优于传统及生成式基准模型，并对分布外场景具有更强的鲁棒性。
conclusion: 通过环境语义增强和流匹配技术，MultiCE-Flow为环境感知通信系统提供了一种高效且精准的信道估计方案。
---
