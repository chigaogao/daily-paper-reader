...
tags: ["query:eeg-ai"]
score: 6.0
evidence: 具有全局时间结构的序列生成模型
tldr: 针对高波动和弱周期性时间序列生成中全局结构与局部变化难以平衡的问题，本文提出了一种结构-残差视角，将时间序列视为结构骨架与随机动态的结合。通过引入分位数转移图来捕捉全局分布和时间依赖，并开发了Graph2TS模型，即一种基于分位数图条件的VAE，实现了从结构图到时间序列的跨模态生成。实验证明该方法在分布保真度和时间对齐方面优于扩散模型和GAN等基准。
selection_source: fresh_fetch
motivation: 现有生成模型在处理高波动时间序列时，往往难以在保持全局时间结构与建模随机局部变化之间取得平衡。
method: 提出Graph2TS模型，利用分位数转移图表征全局结构，并将其作为VAE的条件来引导时间序列的生成。
result: 在太阳黑子、电力负荷、心电图和脑电图等数据集上，该方法在分布保真度、时间对齐和代表性方面均优于扩散和GAN模型。
conclusion: 将结构控制与跨模态生成相结合，为解决复杂时间序列建模问题提供了一个极具前景的新方向。
---
