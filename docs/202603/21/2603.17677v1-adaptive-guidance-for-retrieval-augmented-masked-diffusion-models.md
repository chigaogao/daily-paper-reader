...
motivation: 旨在解决检索增强生成在掩码扩散模型中因检索内容噪声或与模型先验冲突而导致的生成质量退化问题。
method: 提出ARAM框架，利用去噪过程中分布偏移的信噪比动态校准引导尺度，实现对检索信息的自适应集成。
result: 在多个知识密集型问答基准测试中，ARAM的表现均优于竞争性的RAG基准模型。
