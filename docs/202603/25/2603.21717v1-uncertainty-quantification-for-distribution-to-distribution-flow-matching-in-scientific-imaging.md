...
## 摘要
分布到分布生成模型支持从建模细胞扰动响应到跨条件医学图像转换的各种科学成像任务。可信的生成既需要可靠性（跨实验室、设备和实验条件的泛化能力），也需要问责制（检测预测可能不可靠的分布外情况）。基于不确定性量化（UQ）的方法是完成这些任务的有潜力的方案，但针对分布到分布生成模型的 UQ 研究仍不充分。我们提出了一个统一的 UQ 框架——贝叶斯随机流匹配（BSFM），该框架解耦了偶然不确定性和认知不确定性。其中的随机流匹配（SFM）组件通过扩散项增强了确定性流，从而提高了模型对未知场景的泛化能力。针对 UQ，我们开发了一种可扩展的贝叶斯方法——MCD-Antithetic，它将蒙特卡洛 Dropout 与样本高效的对偶采样相结合，为分布外检测生成有效的异常评分。在细胞成像（BBBC021、JUMP）和脑部 fMRI（心理理论）等多种场景下的实验表明，SFM 提高了可靠性，而 MCD-Antithetic 增强了问责制。
